...
def composition_edit(composition_id):
    ...
    if form.validate_on_submit():
        # Update the main composition object's fields
        form.populate_obj(composition)

        # Process the instrumentation string, which will delete old and add new
        # This now just stages the changes in the session
        instrumentation_data = form.instrumentation.data.strip()
        process_chamber_instrumentation_line(composition.id, instrumentation_data)

        # Now, commit everything at once: the composition changes AND the instrumentation changes.
        db.session.commit()
