...
(function () {
  function init(modalEl) {
    // DO NOT require ensembleId at init time.
    // It will be provided by the clicked button (event.relatedTarget).
    ...
    // ✅ critical: capture ensemble_id from the clicked button
    ...
    modalEl.addEventListener('shown.bs.modal', async () => {
      ...
      const ensembleId = getEnsembleId();
      if (!ensembleId) {
        showError('Chybí ensemble_id (data-ensemble-id na tlačítku).');
      ...
      try {
        ...
        if (loadingEl) loadingEl.classList.add('d-none');
        if (contentEl) contentEl.classList.add('d-none');
        showError(`Nepodařilo se načíst informace o převodu. (${e.message})`);
        if (submitBtn) submitBtn.disabled = true;
      }
    });
    ...
    submitBtn?.addEventListener('click', async () => {
      ...
      try {
        ...
        showError(`Nepodařilo se provést převod. (${e.message})`);
        setBusy(false);
      }
    });
  }
